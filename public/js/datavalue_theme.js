...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(this).on("click", ".dropdown-language .dropdown-item", function (event) {
            event.preventDefault();
            let $this = $(this);
            // $this.siblings(".selected").removeClass("selected");
            let language = $this.data('lang');
            let selected_flag = $this.find(".dv-lang-flag").attr("class");
            $(".dropdown-language .dropdown-lang-link").html(`<span class="${selected_flag}"></span> ${language}`);
            frappe.call({
                method: "datavalue_theme.api.change_language",
                args: {
    ...
    $(document).on("app-loaded", function () {
        new Vue({
            ...
            methods: {
                get_company_logo: function () {
                    ...
                    frappe.call({
                        type: 'POST',
                        method: 'datavalue_theme.api.get_company_logo',
                        args: {},
                        callback: function (response) {
                            if (response.message && response.message.length) {
                                ...
                                $this.logo_path = '/assets/datavalue_theme/images/logo-32.png';
                            }
                        }
                    });
                }
        ...
        new Vue({
            ...
            created: function () {
                if (localStorage.getItem("active_lang") && localStorage.getItem("active_lang").length == 2) {
                    this.active_lang = localStorage.getItem("active_lang");
                } else {
                    this.active_lang = 'EN';
                }
            }
