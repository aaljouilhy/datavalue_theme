...
frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
    ...
    async get_report_doc() {
        ...
        let report_doc = await frappe.model.with_doc('Report', this.report_name).then(doc => {
            this.report_doc = doc;
            // promiseResolve(this.report_doc)
        }).then(() => (this.report_doc && this.report_doc.ref_doctype) ? frappe.model.with_doctype(this.report_doc.ref_doctype) : '');

        console.log('RRRRRRRRRRR-report_doc', report_doc);
